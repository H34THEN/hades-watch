import Link from "next/link";
import { AdminEventPanel } from "@/components/admin/AdminEventPanel";
import { RoleBadge } from "@/components/badges/RoleBadge";
import { AccessDenied } from "@/components/layout/AccessDenied";
import { CommandButton } from "@/components/terminal/CommandButton";
import { SystemAlert } from "@/components/terminal/SystemAlert";
import { TerminalPanel } from "@/components/terminal/TerminalPanel";
import { writeAuditLog } from "@/lib/audit";
import { getAllEvents } from "@/lib/actions/events";
import { getHighestRole, isAdmin } from "@/lib/auth/roles";
import { getSessionUser } from "@/lib/auth/session";

export const metadata = { title: "Admin Events" };

export default async function AdminEventsPage() {
  const user = await getSessionUser();

  if (!user) {
    return (
      <AccessDenied
        title="Authentication Required"
        message="Admin console requires authentication."
      />
    );
  }

  if (!isAdmin(user.roles)) {
    await writeAuditLog({
      action: "access.denied.admin",
      actorId: user.id,
    });
    return (
      <AccessDenied
        title="Clearance Insufficient"
        message="Event management requires Owner or Admin clearance."
        requiredRole="Owner / Admin"
      />
    );
  }

  const events = await getAllEvents();

  return (
    <div className="mx-auto max-w-6xl px-4 py-16">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-mono text-3xl tracking-widest uppercase">
            Events Admin
          </h1>
          <p className="text-muted-foreground">
            Schedule operations, briefings, and virtual meetings.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <RoleBadge role={getHighestRole(user.roles)} />
          <Link href="/admin">
            <CommandButton size="sm" variant="outline">← Admin</CommandButton>
          </Link>
        </div>
      </div>

      <SystemAlert
        title="Jitsi Notice"
        message="Jitsi rooms are not private by default. Anyone with the URL can join. For sensitive meetings, use proper access controls."
        variant="warning"
      />

      <TerminalPanel title="event.management" className="mt-6">
        <AdminEventPanel events={events} />
      </TerminalPanel>
    </div>
  );
}
