...
Goal:
Add a new world lore pack for /archive/world and update the archive taxonomy to support five lore categories:
...
Use the Markdown file `docs/lore/WORLD_LORE_PACK_001_SURFACE_BREAKS.md` or create it from the content provided in this prompt.

Important:
- This is fictional Hades Watch lore.
- Do not use real living political figures as characters.
- Transform real-world inspiration into myth-tech fiction.
- Keep the tone cyberpunk Greek-underworld dystopia, political horror, mutual aid, and underworld resistance.
- Do not modify `.env`.
- Do not require email.
- Do not delete production users.
- Do not commit `.next`.

Archive taxonomy:
Add or extend archive category support so `/archive` can filter/display:
- `character-lore`
- `world-lore`
- `faction-lore`
- `mythos-and-ethos`
...
World lore entries to add:
1. The Jackal Ledger
   slug: `the-jackal-ledger`
   category: `world-lore`
   threatClass: `Civic Necrosis`
   related factions: Asclepian Veil, Oracular Circuit, Daedalus Foundry

2. The Bronze Gate Levies
   slug: `the-bronze-gate-levies`
   category: `world-lore`
   threatClass: `Manufactured Hunger`
   related factions: Myrmidon Grinders, Daedalus Foundry, Styx Rats

3. The White Lantern Hunts
   slug: `the-white-lantern-hunts`
   category: `world-lore`
   threatClass: `Civic Exorcism`
   related factions: Asclepian Veil, Myrmidon Grinders, Styx Rats, Oracular Circuit

4. The Two New Fires
   slug: `the-two-new-fires`
   category: `world-lore`
   threatClass: `Foreign Flame`
   related factions: Asclepian Veil, Oracular Circuit, Myrmidon Grinders, Daedalus Foundry

5. The Thunder Casket Posts
   slug: `the-thunder-casket-posts`
   category: `world-lore`
   threatClass: `Annihilation Theater`
   related factions: Styx Rats, Oracular Circuit, Asclepian Veil

Implementation requirements:
- If the LoreEntry model already supports category, summary, content, slug, visibility, and unlock behavior, use existing fields.
- If it does not support the new taxonomy cleanly, add only safe metadata now and document richer metadata for later.
- Seed entries idempotently by slug.
- Prefer a dedicated seed helper if lore seed structure exists.
- If archive pages use static canonical fallback data, add these entries there too.
- Ensure `/archive/world` renders a world-lore filtered view or a category route if route architecture supports it.
- If route does not exist yet, add `/archive/[category]` or `/archive/world` using existing archive UI patterns.
- Add archive cards with title, summary, threat class, related systems, and related factions when metadata is available.
- Add detail pages or ensure existing `/archive/lore/[slug]` can display the full Markdown/content.
...
Commit:
git add .
git commit -m "Add Surface Breaks world lore archive pack"
git push

VPS deploy commands after push:
...
https://hadeswatch.com/archive/world
https://hadeswatch.com/archive/lore/the-jackal-ledger
https://hadeswatch.com/archive/lore/the-bronze-gate-levies
https://hadeswatch.com/archive/lore/the-white-lantern-hunts
https://hadeswatch.com/archive/lore/the-two-new-fires
https://hadeswatch.com/archive/lore/the-thunder-casket-posts
```

---

# Additional State-of-Affairs Bulletin Seeds

These can become short dated entries later.

```json
[
  {
    "slug": "underwatch-advisory-low-lumen-east-market",
    "title": "Underwatch Advisory: Low-Lumen East Market",
    "category": "state-of-affairs",
    "summary": "White Lantern activity detected near hospitality corridors and night transit exits. Use escort routes and avoid solo movement."
  },
  {
    "slug": "casket-weather-red-feed",
    "title": "Casket Weather: Red Feed",
    "category": "state-of-affairs",
    "summary": "Thunder Casket discourse has entered panic amplification. Verify before sharing. Check on vulnerable operatives."
  },
  {
    "slug": "bronze-gate-shortage-medical-filters",
    "title": "Bronze Gate Shortage: Medical Filters",
    "category": "state-of-affairs",
    "summary": "Filter components delayed by tariff choke points. Foundry substitution guide pending Veil review."
  },
  {
    "slug": "jackal-ledger-record-recovery",
    "title": "Jackal Ledger Record Recovery",
    "category": "state-of-affairs",
    "summary": "Dead Index volunteers recovered partial benefit denial records from a decommissioned civic server."
  }
]
```
